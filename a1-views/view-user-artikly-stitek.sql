...
LEFT JOIN Ceniky_PolozkaCeniku AS Cena ON Cena.Artikl_ID = Art.ID AND Cenik_ID = (SELECT TOP 1 VychoziCenik_ID FROM System_AgendaDetail)
LEFT JOIN Sklady_Zasoba AS Zas ON Zas.Artikl_ID = Art.ID AND Zas.Sklad_ID = (SELECT TOP 1 VychoziSklad_ID FROM System_AgendaDetail)
LEFT JOIN Sklady_StavZasoby AS Stav ON Stav.Konto_ID = Zas.ID